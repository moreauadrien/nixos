# Voice-to-text daemon (whisper.cpp via voxtype-vulkan).
{
  ...
  flake.nixosModules.voxtype = moduleWithSystem ({ pkgs, ... }: {
    ...
    # voxtype reads the hotkey directly via evdev (Hyprland can't bind RALT,
    # seen as a MOD5 modifier) -> requires the 'input' group.
    users.users.adrien.extraGroups = [ "input" ];
    ...
    # true: Hyprland can't bind RALT (AltGr is seen as a MOD5 modifier),
    # so we let voxtype read it directly via evdev (requires 'input' group).
    enabled = true
